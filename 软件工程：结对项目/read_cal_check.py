...
def readfile(qstpath):
    ...
    with open(qstpath, 'r', encoding='utf-8') as f:
        file = f.readlines() # 一行读取
    # 删题号
    for single_question in file: # 对于每一行(题目)而言
        line = single_question.split('.')[-1] # 按照'.'进行分割，取'.'后面部分
        line = line.strip('\n') # 去除每行末的换行符，方便后续处理
        file_list.append(line) # 将处理后的每一行文本放入file_list中
    return file_list
...
def check(q_list, a_list):
    ...
    for single in range(len(q_list)):
        if cal(q_list[single]) == cal(a_list[single]): # 该题目结果正确，把题号写入correct_list
            correct_list.append(single+1)
        else: # 该题目结果错误，把题号写入wrong_list
            wrong_list.append(single+1)
...
def cal(formula):
    ...
    for each in range(len(mix)):
        mix_list.append('(' + mix[each].replace('\'', '+') + ')') # 将带分数转化为合适的形式
        formula = formula.replace(mix[each], mix_list[each]) # 替换为处理后的形式，以更新算式字符串
...
# 保存改卷结果(比较结果)到文件
def saveresult_as_file(res_list, savepath='Grade.txt'):
    # 规范输出格式
    def output(out_result):
        formatted_res = ''
        if len(out_result) != 0: # 检查out_result列表是否为空
            formatted_res += ' (' # 如果有结果比较，将字符串'('追加到retstr中
            for each in out_result:
                formatted_res += str(each) # 转换为字符串
...
if __name__ == '__main__':
    ...
    res = check(qst_list, ans_list) # 进行结果的比较，并输出
    saveresult_as_file(res) # 保存阅卷结果到文件